...
static bool frame_to_nv12(const AVFrame *src, video_frame_t *dst) {
    ...
    for (uint32_t row = 0; row < h; row++)
        memcpy(dst->data + row * w, y_src + row * y_stride, w);

    /* Interleave U and V planes into NV12 */
...
bool video_decoder_decode(video_decoder_t *decoder, const uint8_t *data,
    ...
    if (!decoder || !decoder->parser || !decoder->codec_ctx) return false;

    frame->data = NULL;

    /* Feed data through parser for proper NAL framing */
    ...
    while (remaining > 0) {
        ...
        if (parsed_size > 0) {
            ...
            if (avcodec_receive_frame(decoder->codec_ctx, decoder->frame) >= 0) {
                if (got_frame) video_frame_free(frame);
                got_frame = frame_to_nv12(decoder->frame, frame);
            }
        }
    }
